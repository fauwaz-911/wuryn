...
def get_store_by_slug(slug: str) -> dict | None:
    ...
    try:
        result = (
            supabase.table("stores")
            .select("id, name, slug, description, business_type, currency, logo_url, banner_url, primary_color, city, country, active")
            .eq("slug", slug)
            .eq("active", True)
            .single()
            .execute()
...
def get_store_by_id(store_id: str) -> dict | None:
    ...
    try:
        result = (
            supabase.table("stores")
            .select("id, name, slug, description, business_type, phone, email, address, city, country, logo_url, banner_url, primary_color, currency, active, plan, plan_expires_at, created_at, updated_at")
            .eq("id", store_id)
            .single()
            .execute()
        )
...
def get_products(store_id: str, available_only: bool = True) -> list:
    ...
    try:
        query = (
            ...
            .select("id, name, description, price, compare_price, available, featured, stock, wa_display_text, category_id, created_at, updated_at, categories(name, slug)")
...
def get_featured_products(store_id: str, limit: int = 6) -> list:
    ...
    try:
        result = (
            supabase.table("products")
            .select("id, name, price, compare_price, available, featured, product_images(url, is_primary)")
            .eq("store_id", store_id)
            .eq("available", True)
            .eq("featured", True)
            .limit(limit)
...
def get_product_by_id(store_id: str, product_id: str) -> dict | None:
    ...
    try:
        result = (
            supabase.table("products")
            .select("id, name, description, price, compare_price, available, stock, track_stock, category_id, created_at, updated_at, product_images(url, is_primary, alt_text, sort_order), categories(name, slug)")
            .eq("store_id", store_id)
            .eq("id", product_id)
            .single()
            .execute()
...
def find_product_by_name(store_id: str, name: str) -> dict | None:
    ...
    try:
        result = (
            supabase.table("products")
            .select("id, name, description, price, compare_price, available, featured, stock, category_id")
            .eq("store_id", store_id)
            .eq("available", True)
            .ilike("name", f"%{name}%")
            .limit(1)
...
def get_or_create_customer(
    ...
    try:
        ...
        result = (
            ...
            .select("id, phone, email, full_name, source, interests, purchase_intent, total_orders, total_spent, last_seen_at, created_at, updated_at")
            .eq("store_id", store_id)
            .eq("phone", phone)
            .execute()
        )
...
def get_customers(store_id: str, limit: int = 100, offset: int = 0) -> list:
    ...
    try:
        result = (
            supabase.table("customers")
            .select("id, phone, email, full_name, source, interests, purchase_intent, total_orders, total_spent, last_seen_at, created_at")
            .eq("store_id", store_id)
...
def get_conversation(store_id: str, customer_phone: str) -> dict:
    ...
    try:
        result = (
            ...
            .select("id, store_id, customer_phone, state, context, last_message_at")
            .eq("store_id", store_id)
...
def get_orders(
    ...
    try:
        query = (
            supabase.table("orders")
            .select("id, reference, channel, status, subtotal, delivery_fee, total_amount, delivery_address, created_at, updated_at, customers(full_name, phone, email), order_items(product_name, unit_price, quantity, subtotal, products(name))")
            .eq("store_id", store_id)
...
def get_customer_orders(store_id: str, customer_id: str, limit: int = 5) -> list:
    ...
    try:
        result = (
            supabase.table("orders")
            .select("id, reference, status, total_amount, created_at, updated_at, order_items(product_name, unit_price, quantity, subtotal)")
            .eq("store_id", store_id)
...
def ping_database() -> bool:
    """
    Send a minimal query to keep the Supabase free project active.

    Supabase pauses free projects after 7 days of database inactivity.
    This is called by the APScheduler keep-alive job in main.py every 6 days.
    Uses the lightest possible query: fetch one column from one stores row.
    Egress cost: essentially zero (~50 bytes).

    Returns:
        True if ping succeeded, False if database unreachable.
    """
    try:
        supabase.table("stores").select("id").limit(1).execute()
        logger.info("[DB:health] Keep-alive ping OK")
        return True
    except Exception as e:
        logger.error(f"[DB:health] Keep-alive ping FAILED: {e}")
        return False
